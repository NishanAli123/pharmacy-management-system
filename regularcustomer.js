...
const MEDICINE_API = '/api/new-medicines/'; // Changed endpoint for medicine details
const availableQuantities = {}; // Store available quantities

async function fetchNextIds() {
    try {
        const response = await fetch('/api/new-next-ids'); // Changed endpoint for fetching IDs
        const result = await response.json();
        if (result.success) {
            document.getElementById('saleId').value = result.ids.saleId; // Set Sale ID
        } else {
            console.error('Failed to fetch IDs:', result.message);
        }
    } catch (error) {
...
function addOrderRow() {
    ...
    orderCount++;
    const orderRow = createOrderRow(orderCount);
    document.getElementById('orderRows').appendChild(orderRow);
    calculateTotalPrice();
}

function createOrderRow(rowNumber) {
    const orderRow = document.createElement('div');
    orderRow.className = 'order-row';
    orderRow.innerHTML = `
        <div class="input-group">
            <input type="text" name="medicineId_${rowNumber}" placeholder="Medicine ID" required>
            <input type="number" name="quantity_${rowNumber}" placeholder="Quantity" required min="1">
        </div>
        <div class="input-group">
            <input type="text" name="medicineName_${rowNumber}" placeholder="Medicine Name" readonly>
            <input type="text" name="price_${rowNumber}" placeholder="Price" readonly>
            <input type="text" name="total_${rowNumber}" placeholder="Total" readonly>
        </div>
...
function updateMedicineRow(data, rowNumber) {
    ...
    document.getElementsByName(`price_${rowNumber}`)[0].value = price; 
...
function calculateTotalPrice() {
    ...
    discountedPrice = totalPrice * 0.95; // Calculate 5% discount
    updateTotalDisplay();
}

function updateTotalDisplay() {
    document.getElementById('totalPrice').innerText = `₹${totalPrice.toFixed(2)}`;
    document.getElementById('discountedPrice').innerText = `₹${discountedPrice.toFixed(2)}`; // Update discounted price
}
...
// Handle quantity input changes for real-time updates
...
document.getElementById('orderForm').addEventListener('submit', async function(e) {
    ...
    // Send order data to new server endpoint
    const response = await fetch('/new-process-order', { // Changed endpoint
        method: 'POST',
        headers: {
...
document.addEventListener('DOMContentLoaded', function() {
    fetchNextIds(); // Fetch IDs on page load
});

function addr() {
    window.location.href = 'rc1.html';
  }
